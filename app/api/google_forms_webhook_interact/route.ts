...
export async function POST(request: NextRequest) {
    ...
    const { email, lastname, firstname } = payload;
    ...
    try {
        const validatedData = ticketSchema.parse({email, lastname, firstname });
        const eventUuid = '97be7609-31a3-45e6-8031-1b6e4dd3f704';
        const reservation = true;
        const sellerUuid = 'e034587b-c11f-4bda-a5f5-ca9b0e07b366';
        const guestsCount = 1;
        ...
        const ticketCount = await db.select()
            .from(eventCustomers)
            .where(eq(eventCustomers.eventUuid, eventUuid))
            .execute();
        ...
        await Promise.all([updateCustomerPromise]);
